...
_collection = _client.get_or_create_collection(name="studentai_documents", metadata={"hnsw:space":"cosine"})

async def embed(text: str) -> list[float]:
    if not GEMINI_API_KEY:
        raise RuntimeError("GEMINI_API_KEY is required for semantic indexing")
...
async def index_chunks(document_id: str, chunks: list[dict[str, Any]]) -> int:
    ...
    for i, chunk in enumerate(chunks):
        ...
        ids.append(hashlib.sha256(f"{document_id}:{i}".encode()).hexdigest())
        texts.append(text); metas.append({"document_id":document_id,"page":chunk.get("page",1),"chunk":i})
        vectors.append(await embed(text))
    if ids:
        _collection.upsert(ids=ids, documents=texts, metadatas=metas, embeddings=vectors)
    return len(ids)
...
async def semantic_search(query: str, document_id: str | None = None, top_k: int = 5) -> list[dict[str, Any]]:
    ...
    docs=result.get("documents", [[]])[0]; metas=result.get("metadatas", [[]])[0]; distances=result.get("distances", [[]])[0]
